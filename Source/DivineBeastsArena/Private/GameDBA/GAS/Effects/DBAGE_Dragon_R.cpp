...
// GameplayEffect - 龙终极技能

#include "GameDBA/GAS/Effects/DBAGE_Dragon_R.h"
#include "GameDBA/GAS/Attributes/DBABattleAttributeSet.h"
#include "Engine/DataTable.h"

UDBAGE_Dragon_R::UDBAGE_Dragon_R()
{
	// 从技能数据表加载配置
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBAGE_Dragon_R");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Dragon_R")), ContextString, false);
		if (SkillRow)
		{
			// 伤害修饰符
