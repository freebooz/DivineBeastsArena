...
// GameplayEffect - 猴终极技能

#include "GameDBA/GAS/Effects/DBAGE_Monkey_R.h"
#include "GameDBA/GAS/Attributes/DBABattleAttributeSet.h"
#include "Engine/DataTable.h"

UDBAGE_Monkey_R::UDBAGE_Monkey_R()
{
	// 从技能数据表加载配置
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBAGE_Monkey_R");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Monkey_R")), ContextString, false);
		if (SkillRow)
		{
			// 伤害修饰符
