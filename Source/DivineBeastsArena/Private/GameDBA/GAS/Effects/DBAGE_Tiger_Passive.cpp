...
// GameplayEffect - 虎被动技能

#include "GameDBA/GAS/Effects/DBAGE_Tiger_Passive.h"
#include "GameDBA/GAS/Attributes/DBABattleAttributeSet.h"
#include "Engine/DataTable.h"

UDBAGE_Tiger_Passive::UDBAGE_Tiger_Passive()
{
	// 从技能数据表加载配置
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBAGE_Tiger_Passive");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Tiger_Passive")), ContextString, false);
		if (SkillRow)
		{
			// 伤害修饰符
