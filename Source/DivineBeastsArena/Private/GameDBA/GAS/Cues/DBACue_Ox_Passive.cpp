...
#include "GameDBA/GAS/Cues/DBACue_Ox_Passive.h"
#include "GameDBA/GAS/DBAAbilitySystemComponent.h"
#include "Kismet/GameplayStatics.h"

ADBACue_Ox_Passive::ADBACue_Ox_Passive()
{
	// 默认配置
...
void ADBACue_Ox_Passive::LoadSkillData()
{
	// 从 DataTable 加载技能数据
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Ox_Passive");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Ox_Passive")), ContextString, false);
		if (SkillRow)
		{
...
bool ADBACue_Ox_Passive::OnExecuteGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	// 获取技能数据
...
void ADBACue_Ox_Passive::OnActiveGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	// 获取技能数据，检查是否有引导特效
	FDBASkillDataRow* SkillData = nullptr;
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
...
void ADBACue_Ox_Passive::OnRemoveGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->IsActive())
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Ox_Passive"))))
				{
					Particle->Deactivate();
				}
			}
