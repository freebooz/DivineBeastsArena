...
#include "GameDBA/GAS/Cues/DBACue_Snake_W.h"
#include "GameDBA/GAS/DBAAbilitySystemComponent.h"
#include "Kismet/GameplayStatics.h"
#include "Particles/ParticleSystemComponent.h"

ADBACue_Snake_W::ADBACue_Snake_W()
{
	// 加载技能数据
	LoadSkillData();
}

void ADBACue_Snake_W::LoadSkillData()
{
	// 从DataTable加载技能数据
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Snake_W");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Snake_W")), ContextString, false);
		if (SkillRow)
		{
...
bool ADBACue_Snake_W::OnExecuteGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	// 获取技能数据
...
void ADBACue_Snake_W::OnActiveGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	// 获取技能数据，检查是否有引导特效
	FDBASkillDataRow* SkillData = nullptr;
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
...
void ADBACue_Snake_W::OnRemoveGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->IsActive())
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Snake_W"))))
				{
					Particle->Deactivate();
				}
			}
