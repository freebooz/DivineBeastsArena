...
#include "GameDBA/GAS/Cues/DBACue_Dog_W.h"
...
void ADBACue_Dog_W::LoadSkillData()
{
	...
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Dog_W");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Dog_W")), ContextString, false);
		if (SkillRow)
		{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->IsActive())
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Dog_W"))))
				{
					Particle->Deactivate();
				}
			}
