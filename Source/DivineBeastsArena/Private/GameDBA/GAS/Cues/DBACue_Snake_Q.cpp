...
#include "GameDBA/GAS/Cues/DBACue_Snake_Q.h"
#include "GameDBA/GAS/DBAAbilitySystemComponent.h"
#include "Kismet/GameplayStatics.h"
#include "Particles/ParticleSystemComponent.h"

ADBACue_Snake_Q::ADBACue_Snake_Q()
{
	// 默认配置
...
void ADBACue_Snake_Q::LoadSkillData()
{
	// 从 DataTable 加载技能数据
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Snake_Q");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Snake_Q")), ContextString, false);
		if (SkillRow)
		{
...
bool ADBACue_Snake_Q::OnExecuteGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	// 获取技能数据
...
void ADBACue_Snake_Q::OnActiveGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	// 获取技能数据，检查是否有引导特效
	FDBASkillDataRow* SkillData = nullptr;
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
...
void ADBACue_Snake_Q::OnRemoveGameplayCue(AActor* Target, const FGameplayCueParameters& Parameters)
{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->IsActive())
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Snake_Q"))))
				{
					Particle->Deactivate();
				}
			}
