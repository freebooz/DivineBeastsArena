...
#include "DBA/GAS/Cues/DBACue_Rabbit_Passive.h"
#include "DBA/GAS/DBAAbilitySystemComponent.h"
#include "Data/DBADataAsset.h"
#include "Kismet/GameplayStatics.h"

UDBACue_Rabbit_Passive::UDBACue_Rabbit_Passive()
{
	// 默认配置
...
void UDBACue_Rabbit_Passive::LoadSkillData()
{
	// 从 DataTable 加载技能数据
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Rabbit_Passive");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Rabbit_Passive")), ContextString, false);
		if (SkillRow)
		{
...
bool UDBACue_Rabbit_Passive::OnExecuteGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	// 获取技能数据
...
void UDBACue_Rabbit_Passive::OnActiveGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	// 获取技能数据，检查是否有引导特效
	FDBASkillDataRow* SkillData = nullptr;
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
...
void UDBACue_Rabbit_Passive::OnRemoveGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->bIsActive)
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Rabbit_Passive"))))
				{
					Particle->Deactivate();
				}
			}
