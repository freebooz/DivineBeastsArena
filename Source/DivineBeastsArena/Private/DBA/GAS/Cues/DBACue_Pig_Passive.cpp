...
#include "DBA/GAS/Cues/DBACue_Pig_Passive.h"
#include "DBA/GAS/DBAAbilitySystemComponent.h"
#include "Data/DBADataAsset.h"
#include "Kismet/GameplayStatics.h"

UDBACue_Pig_Passive::UDBACue_Pig_Passive()
{
	// 默认配置
...
void UDBACue_Pig_Passive::LoadSkillData()
{
	// 从 DataTable 加载技能数据
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Pig_Passive");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Pig_Passive")), ContextString, false);
		if (SkillRow)
		{
...
bool UDBACue_Pig_Passive::OnExecuteGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	// 获取技能数据
...
void UDBACue_Pig_Passive::OnActiveGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	// 获取技能数据，检查是否有引导特效
	FDBASkillDataRow* SkillData = nullptr;
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
...
void UDBACue_Pig_Passive::OnRemoveGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->bIsActive)
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Pig_Passive"))))
				{
					Particle->Deactivate();
				}
			}
