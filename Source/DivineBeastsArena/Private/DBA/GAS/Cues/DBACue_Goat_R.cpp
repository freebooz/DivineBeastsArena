...
void UDBACue_Goat_R::LoadSkillData()
{
	// 从 DataTable 加载技能数据
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Goat_R");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Goat_R")), ContextString, false);
		if (SkillRow)
		{
...
bool UDBACue_Goat_R::OnExecuteGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	// 获取技能数据
...
void UDBACue_Goat_R::OnActiveGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	// 获取技能数据，检查是否有引导特效
	FDBASkillDataRow* SkillData = nullptr;
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
...
void UDBACue_Goat_R::OnRemoveGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->bIsActive)
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Goat_R"))))
				{
					Particle->Deactivate();
				}
			}
