...
void UDBACue_Goat_Q::LoadSkillData()
{
	// 从 DataTable 加载技能数据
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Goat_Q");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Goat_Q")), ContextString, false);
		if (SkillRow)
		{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->bIsActive)
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Goat_Q"))))
				{
					Particle->Deactivate();
				}
			}
