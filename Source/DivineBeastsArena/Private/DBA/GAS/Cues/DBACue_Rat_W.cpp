...
void UDBACue_Rat_W::LoadSkillData()
{
	// 从 DataTable 加载技能数据
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
	if (SkillTable)
	{
		static const FString ContextString = TEXT("DBACue_Rat_W");
		FDBASkillDataRow* SkillRow = SkillTable->FindRow<FDBASkillDataRow>(FName(TEXT("Rat_W")), ContextString, false);
		if (SkillRow)
		{
...
bool UDBACue_Rat_W::OnExecuteGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	// 获取技能数据
...
void UDBACue_Rat_W::OnActiveGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	// 获取技能数据，检查是否有引导特效
	FDBASkillDataRow* SkillData = nullptr;
	UDataTable* SkillTable = LoadObject<UDataTable>(nullptr, TEXT("DataTable'/Game/Data/Skills/SkillDataTable.SkillDataTable'"));
...
void UDBACue_Rat_W::OnRemoveGameplayCue(AActor* Target, FGameplayCueParameters& Parameters)
{
	...
	if (Target)
	{
		...
		for (UParticleSystemComponent* Particle : ParticleComponents)
		{
			if (Particle && Particle->bIsActive)
			{
				...
				if (Particle->ComponentHasTag(FName(TEXT("Cue_Rat_W"))))
				{
					Particle->Deactivate();
				}
			}
