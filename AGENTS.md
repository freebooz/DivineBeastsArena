...
- 默认非交互执行：可验证的读取、文档同步、源码小步修改、编译、静态检查和看板更新均直接推进；自动测试、脚本验证和业务验收受“全局人工审核验证策略”与“全局自动化脚本禁令”约束，不属于默认直接执行范围。
...
## 全局人工审核验证策略
- PolicyId: `DBA.Validation.ManualReviewOnly`
- RuntimePolicyId: `DBA.Validation.ManualRuntimeReview`
- 本策略覆盖“全局自主执行策略”中关于自动测试、脚本验证和自动化验收的默认授权。除非用户当次明确书面解除，否则禁止自动测试、自动判定通过或以脚本断言替代人工审核。
- 默认审核方式为“运行人工审核”：Agent 负责准备并启动可见的 API 服务、Dedicated Server、Editor、PIE 或客户端窗口，人工负责输入、点击、角色选择、场景观察、联机操作和最终结论；不得用进程存在、窗口打开、日志无错误或截图生成自动推定业务通过。
- 对用户可见功能、交互流程、画面布局、动画、音频、网络联机和完整业务链路，优先进行运行时人工审核；除非用户当次明确书面要求，否则不主动执行自动化测试、自动化回归、自动截图判定或自动验收脚本。
- 禁止运行会自动完成业务验收的测试流程，包括但不限于自动登录、自动创建/选择角色、自动匹配、自动 ClientTravel、自动点击 UI、自动启动双客户端联机回归，以及使用 `DBASkipFrontendFlow`、`DBAAutoLobbyFlow`、`DBAAutoMatchFlow` 等参数绕过人工前端流程。
- 不得将 `start-local-ue-validation.ps1`、`start-local-frontend-match-validation.ps1`、`start-local-ue-frontend-match-validation.ps1` 或同类脚本的自动断言结果表述为人工验收、登录流程验收或用户体验验收。
- 需要验证时，Agent 只可按用户要求准备服务、独立服务器、客户端窗口、日志与截图条件；登录、选角、进入大厅、交互、网络连接等验收步骤必须由人工可见地审核，并记录操作步骤、画面证据、日志证据和结论。
- 运行人工审核时，Agent 必须明确区分“环境已就绪”“人工正在审核”“人工确认通过”“人工发现失败”四种状态；未收到或未记录人工结论时，只能报告环境已就绪，不得报告验收通过。
- 编译、静态检查、格式检查和只读诊断不属于业务验收；可以在不触发游戏流程的前提下执行，但结果必须标明为“工程检查”，不得替代人工验证结论。
- 人工审核发现流程异常时，必须立即停止后续自动化动作，保留现场日志与截图，先修复流程逻辑，再由人工重新审核。

## 全局自动化脚本禁令
- PolicyId: `DBA.AutomationScript.Prohibited`
- 项目禁止保留、创建、恢复、执行或引用任何自动化脚本，包括 PowerShell、Python、Shell、Batch、CMD、Editor Python、自动化测试、自动化启动、自动化发布、自动化导入、自动化验收和自动化证据采集脚本。
- 禁止保留任何可绕过人工业务流程的命令行入口、运行时开关或 C++ 自动流程，包括自动登录、自动游客登录、自动创建/选择角色、自动组队、自动匹配、自动进服、自动 ClientTravel、前端流程跳过、Mock 账户强制切换和自动存档隔离入口。
- 后续工程验证仅允许人工审核：准备运行环境后，由人工完成 UI 输入、角色选择、进入大厅、联机和交互；Agent 仅记录人工可见的截图、日志和结论，不得自动代操作或替代验收。
- 发现自动化脚本、自动化入口或其文档引用时，必须直接删除并同步移除调用关系；不得以兼容层、废弃标记、隐藏开关或保留空壳形式继续存在。
...
- `docs/`：方案、架构、发布与人工审核文档；禁止保留 `scripts/` 自动化目录。
...
- Compile。
- Save。
- 运行人工审核：按需启动可见 PIE 或独立客户端，由人工完成操作、观察并给出结论；禁止自动操作与自动判定。
- Git Diff。
...
## 测试规范
- 默认不自动运行测试或自动化审核；测试代码可以保留作为工程契约，只有用户当次明确书面要求时才执行。
...
- 提交前默认执行涉及模块的编译与静态检查；自动测试是否执行以用户当次明确书面指令为准。人工业务审核结果必须独立记录，不得由测试结果替代。
