...
默认非交互执行：可验证的读取、文档同步、源码小步修改、编译、静态检查和看板更新均直接推进；自动测试、脚本验证和业务验收不属于默认直接执行范围，统一受人工审核策略约束。
...
对不依赖外部输入的工程推进项，优先直接执行，包括文档同步、源码小步修复、测试契约设计、编译与静态检查、中文日志迁移和阶段看板更新；不得自动运行测试或业务审核。
...
### 2.1.0 全局自动化脚本禁令与人工审核验证策略

PolicyId: `DBA.AutomationScript.Prohibited`

人工运行审核 PolicyId: `DBA.Validation.ManualRuntimeReview`

本策略覆盖本总控提示词中所有关于自动化测试、自动化验证、自动化启动、自动化发布、自动化导入、自动化证据采集和自动化工程流程的既有表述。项目禁止保留、创建、恢复、执行或引用任何自动化脚本，包括 PowerShell、Python、Shell、Batch、CMD、Editor Python、CI 自动化脚本和自动化测试脚本。

项目禁止保留可绕过人工业务流程的运行时入口和命令行开关，包括自动登录、自动游客登录、自动创建/选择角色、自动组队、自动匹配、自动进服、自动 ClientTravel、跳过前端流程、强制 Mock 账户和自动存档隔离。不得以兼容层、废弃注释、隐藏开关或空实现形式保留这些路径。

所有业务验证必须改为人工审核。Agent 仅可准备服务、独立服务器、客户端窗口、日志和截图条件；登录、选角、创建角色、进入大厅、联机、交互和发布验收必须由人工可见地审核，并记录人工操作、画面证据、日志证据和结论。编译与只读静态检查仅是工程检查，不能替代人工验收。

默认采用“运行人工审核”：Agent 可以启动可见的 API 服务、Dedicated Server、Unreal Editor、PIE 或客户端窗口并准备日志，但所有输入、点击、选角、场景观察、联机操作和最终判定均由人工完成。进程存在、窗口打开、日志无错误、截图生成、编译成功或服务端口可用只能证明环境已就绪，不得自动推定业务通过。

对用户可见功能、交互流程、界面布局、动画、音频、网络联机和完整业务链路，应优先进行运行时人工审核。除非用户当次明确书面要求，否则 Agent 不主动执行自动化测试、自动化回归、自动点击、自动截图判定或自动验收脚本。未记录人工结论时，状态只能写为“环境已就绪，等待人工审核”，不得写为“验收通过”。
