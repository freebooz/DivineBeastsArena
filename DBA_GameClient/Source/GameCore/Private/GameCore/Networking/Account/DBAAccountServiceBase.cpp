...
#include "GameCore/Data/Profile/DBASaveGameVersions.h"
...
UDBAAccountServiceBase* UDBAAccountServiceBase::Resolve(UGameInstance* GameInstance)
{
	if (!GameInstance)
	{
		return nullptr;
	...
	const TArray<UDBAAccountServiceBase*> Services = GameInstance->GetSubsystemArrayCopy<UDBAAccountServiceBase>();
	for (UDBAAccountServiceBase* Service : Services)
	{
		if (IsValid(Service) && Service->IsLoggedIn())
		{
			return Service;
		}
	}
