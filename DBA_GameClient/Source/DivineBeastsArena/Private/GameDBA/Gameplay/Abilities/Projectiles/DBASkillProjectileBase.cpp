...
ADBASkillProjectileBase::ADBASkillProjectileBase()
{
	...
	{
		ProjectileFallbackCore->SetStaticMesh(SphereMesh);
	}

	TSoftObjectPtr<UMaterialInterface> FallbackMaterialRef(FSoftObjectPath(TEXT("/Engine/BasicShapes/BasicShapeMaterial.BasicShapeMaterial")));
	if (UMaterialInterface* FallbackMaterial = FallbackMaterialRef.Get())
	{
		ProjectileFallbackMaterial = FallbackMaterial;
	}
