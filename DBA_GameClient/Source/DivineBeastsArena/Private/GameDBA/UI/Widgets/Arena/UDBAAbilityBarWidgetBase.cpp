...
#include "GameDBA/UI/Widgets/Arena/AbilityBar/DBAAbilitySlotWidget.h"
...
void UDBAAbilityBarWidgetBase::NativeDestruct()
{
	...
	UnbindFromOwningPlayerPawnEvent();
...
void UDBAAbilityBarWidgetBase::BindToCharacter(ADBAZodiacCharacterBase* InCharacter)
{
	...
	if (InCharacter)
	{
		...
		if (UDBAPlayableSkillComponent* SkillComponent = InCharacter->GetPlayableSkillComponent())
		{
			SkillComponent->OnSkillCatalogChanged.AddDynamic(this, &UDBAAbilityBarWidgetBase::HandleSkillCatalogChanged);
			SkillCatalogEventComponent = SkillComponent;
		}
	}
	RefreshSkillCatalog();
	RefreshCooldowns();
...
void UDBAAbilityBarWidgetBase::HandleSkillCatalogChanged(const FDBAPlayableSkillCatalogSummary& Summary)
{
	RefreshSkillCatalog();
	RefreshCooldowns();
}
...
void UDBAAbilityBarWidgetBase::UnbindFromCooldownEvents()
{
	...
	if (UDBAPlayableSkillComponent* SkillComponent = SkillCatalogEventComponent.Get())
	{
		SkillComponent->OnSkillCatalogChanged.RemoveDynamic(this, &UDBAAbilityBarWidgetBase::HandleSkillCatalogChanged);
	}
	SkillCatalogEventComponent.Reset();
}

void UDBAAbilityBarWidgetBase::BindToOwningPlayerPawnEvent()
{
	APlayerController* PlayerController = GetOwningPlayer();
	if (!PlayerController || PawnEventController.Get() == PlayerController)
	{
		return;
	}

	UnbindFromOwningPlayerPawnEvent();
	PlayerController->OnPossessedPawnChanged.AddDynamic(this, &UDBAAbilityBarWidgetBase::HandleOwningPawnChanged);
	PawnEventController = PlayerController;
}

void UDBAAbilityBarWidgetBase::UnbindFromOwningPlayerPawnEvent()
{
	if (APlayerController* PlayerController = PawnEventController.Get())
	{
		PlayerController->OnPossessedPawnChanged.RemoveDynamic(this, &UDBAAbilityBarWidgetBase::HandleOwningPawnChanged);
	}
	PawnEventController.Reset();
}

void UDBAAbilityBarWidgetBase::HandleOwningPawnChanged(APawn* OldPawn, APawn* NewPawn)
{
	BindToCharacter(Cast<ADBAZodiacCharacterBase>(NewPawn));
}
