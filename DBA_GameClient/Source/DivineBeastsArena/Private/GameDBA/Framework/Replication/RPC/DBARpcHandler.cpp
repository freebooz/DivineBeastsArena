...
bool ADBARpcHandler::ServerMoveTo_Validate(FVector_NetQuantize10 Location)
{
	...
	if (OwnerActor)
	{
		const float MoveDistance = FVector::Dist(OwnerActor->GetActorLocation(), Location);
		if (MoveDistance > DBAConstants::MaxMoveDistancePerRequest)
		{
			UE_LOG(LogDBANetwork, Warning, TEXT("[服务器] 移动被拒绝：移动距离 %.1f 超过上限 %.1f。"), MoveDistance, DBAConstants::MaxMoveDistancePerRequest);
			return false;
		}
	}
...
bool ADBARpcHandler::ServerRequestAttack_Validate()
{
	...
	if (LastAttackRequestTime > 0.0f && (CurrentTime - LastAttackRequestTime) < DBAConstants::MinAttackInterval)
	{
		UE_LOG(LogDBANetwork, Warning, TEXT("[服务器] 攻击被拒绝：请求间隔 %.3f 秒小于下限 %.3f 秒。"), CurrentTime - LastAttackRequestTime, DBAConstants::MinAttackInterval);
		return false;
	}
