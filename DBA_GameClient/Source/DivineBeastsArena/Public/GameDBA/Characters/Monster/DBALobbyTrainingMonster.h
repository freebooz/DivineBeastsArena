...
protected:
	...
	/**
	 * 响应 CurrentHealth 属性变化（替代旧 OnRep_CurrentHealth 机制）。
	 * 通过 ASC 注册 AttributeChange Delegate 驱动血条 UI 更新，
	 * 符合《DBA.UI.EventAsync》事件驱动 UI 更新策略。
	 */
...
private:
	...
	void RequestLobbyMonsterVisuals();
	void ApplyLoadedLobbyMonsterMesh(USkeletalMesh* LoadedMesh);
	void ApplyLobbyMonsterTintMaterial();
	void ApplyLobbyMonsterSelectionRing();
...
private:
	...
	UPROPERTY(Transient)
	TObjectPtr<UMaterialInterface> LobbyTintMaterial;

	UPROPERTY(Transient)
	TObjectPtr<UStaticMesh> LobbySelectionRingMesh;
