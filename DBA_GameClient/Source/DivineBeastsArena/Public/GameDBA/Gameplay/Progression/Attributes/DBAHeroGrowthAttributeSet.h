 ...
 * - HeroLevel：英雄等级
 * - Experience：当前经验值
 * - ExperienceToNextLevel：升至下一级所需经验
 * - RespawnTime：复活时间（秒）
 * - GoldBounty：击杀奖励金币
 *
 * 设计原则：
 ...
 * - 成长属性通过经验结算系统修改
 * - 不影响核心战斗公式
 * - 默认值由 UDBAHeroGrowthDefaultsDataAsset 驱动，构造函数不再硬编码
 *
 * 数据驱动链路：
 *   ADBAPlayerState::BeginPlay
 *     → RequestDefaultHeroGrowthDefaultsAsync（读 DeveloperSettings 软引用）
 *     → DBAAsyncAssetLoader::RequestAsyncAsset 异步加载
 *     → HandleDefaultHeroGrowthDefaultsLoaded
 *     → UDBAHeroGrowthAttributeSet::ApplyDefaultAttributes
 */
UCLASS()
...
public:
	...
	/**
	 * 从数据资产应用默认英雄成长属性
	 * 由 ADBAPlayerState 在异步加载完成后调用
	 * @param DefaultsData 英雄成长属性默认值数据资产
	 */
	void ApplyDefaultAttributes(const class UDBAHeroGrowthDefaultsDataAsset* DefaultsData);
