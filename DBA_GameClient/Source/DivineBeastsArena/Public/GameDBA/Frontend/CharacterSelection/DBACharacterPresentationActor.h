...
struct DIVINEBEASTSARENA_API FDBACharacterPresentationStageSpec
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "DBA|CharacterPresentation")
	float FaceLightIntensity = 5000.0f;

	UPROPERTY(BlueprintReadOnly, Category = "DBA|CharacterPresentation")
	float SkyLightIntensity = 3.0f;

	UPROPERTY(BlueprintReadOnly, Category = "DBA|CharacterPresentation")
	float PillarRedLightIntensity = 1200.0f;
...
public:
	...
	/** 查找当前固定关卡中已由关卡设计师放置的角色展示舞台，不在运行时生成。 */
	static ADBACharacterPresentationActor* FindPlacedPresentationStage(UWorld* World);
	...
	static FLinearColor GetPreviewAccentTintForZodiac(EDBAZodiac Zodiac);
	static bool UsesTintedPlaceholderMesh();
