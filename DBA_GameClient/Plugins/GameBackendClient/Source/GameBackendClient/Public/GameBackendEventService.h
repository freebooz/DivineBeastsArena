...
#include "GameBackendEventService.generated.h"

class UDBA_GameBackendClientSubsystem;
class FDBA_GameBackendHttpClient;

UCLASS(BlueprintType)
class GAMEBACKENDCLIENT_API UDBA_GameBackendEventService : public UObject
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "DBA_GameBackend|Event")
	void GetEvents(const FDBA_GameBackendResponseDelegate& Callback);

	UFUNCTION(BlueprintCallable, Category = "DBA_GameBackend|Event")
	void GetMyEventProgress(const FDBA_GameBackendResponseDelegate& Callback);
