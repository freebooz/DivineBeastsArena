...
#include "DBA_GameBackendCrashService.generated.h"

class UDBA_GameBackendClientSubsystem;
class FDBA_GameBackendHttpClient;

UCLASS(BlueprintType)
class GAMEBACKENDCLIENT_API UDBA_GameBackendCrashService : public UObject
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "DBA_GameBackend|Crash")
	void ScanCrashFiles();

	UFUNCTION(BlueprintCallable, Category = "DBA_GameBackend|Crash")
	void UploadCrashReport(const FString& CrashFilePath, const FDBA_GameBackendResponseDelegate& Callback);

	UFUNCTION(BlueprintCallable, Category = "DBA_GameBackend|Crash")
	void UploadClientLog(const FString& LogFilePath, const FDBA_GameBackendResponseDelegate& Callback);
