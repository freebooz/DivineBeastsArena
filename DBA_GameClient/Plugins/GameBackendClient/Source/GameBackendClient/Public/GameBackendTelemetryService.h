...
#include "DBA_GameBackendTelemetryService.generated.h"

class UDBA_GameBackendClientSubsystem;
class FDBA_GameBackendHttpClient;

UCLASS(BlueprintType)
class GAMEBACKENDCLIENT_API UDBA_GameBackendTelemetryService : public UObject
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "DBA_GameBackend|Telemetry")
	void TrackEvent(const FString& EventName, const TMap<FString, FString>& Properties);

	UFUNCTION(BlueprintCallable, Category = "DBA_GameBackend|Telemetry")
	void TrackBatch(const TArray<FDBA_GameBackendTelemetryEvent>& Events);

	UFUNCTION(BlueprintCallable, Category = "DBA_GameBackend|Telemetry")
	void Flush();

	UFUNCTION(BlueprintPure, Category = "DBA_GameBackend|Telemetry")
...
private:
	...
	FString BuildBatchPayload(const TArray<FDBA_GameBackendTelemetryEvent>& Events) const;
