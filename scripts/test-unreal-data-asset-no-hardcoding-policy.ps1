<#
Validates the global DataAsset / no-hardcoding policy.
...
function Assert-FileContains {
  ...
  if ($missingTokens.Count -gt 0) {
    throw "$Path is missing DataAsset / no-hardcoding policy tokens: $($missingTokens -join ', ')"
  }
}
...
Write-Host "PASS: Unreal DataAsset / no-hardcoding policy contract" -ForegroundColor Green
